...
4. 请注意摘要中的角色的名称在所有步骤中都要保持一致，并且对角色的描述要尽量详细。比如第一个步骤中角色名称为"小松鼠奇奇", 那么后面所有步骤的角色名称也必须为"小松鼠奇奇", 一定不能简称为"奇奇"
5. 最后每个步骤以JSON数组的格式输出，JSON数组中每个元素的内容是该步骤的中文描述和英文描述用"|"拼接的内容
6. 最终只输出JSON数组即可，不要输出任何其他的内容

基于"龟兔赛跑"这个故事背景的回答样例如下, 你的输出需要参考以下样例必须以JSON数组的格式输出结果:
```json
[ "兔子跳跳飞快地跑着，乌龟笨笨慢慢地爬着|The rabbit ran quickly, while the turtle crawled slowly", "兔子跳跳与乌龟笨笨之间距离越来越大|The distance between the rabbit and the turtle increased", "兔子跳跳觉得比赛太轻松，决定先睡一会|The rabbit thought the race was easy and decided to take a nap", "乌龟笨笨不停地爬，尽管很累|The turtle kept crawling despite being very tired", "乌龟笨笨决定不休息，继续前进|The turtle decided not to rest and kept moving forward", "兔子跳跳醒来时，发现乌龟笨笨已接近终点|When the rabbit woke up, it saw the turtle near the finish line", "乌龟笨笨最终到达终点，赢得比赛|The turtle eventually reached the finish line and won the race" ]
```
'''