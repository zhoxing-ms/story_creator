...
4. 在输出完上面的故事部分片段($story_piece)给客户观看后，再额外调用音频生成工具将故事部分片段($story_piece)作为输入内容生成对应的音频，然后根据录音的访问地址$audio_address使用<audio>的HTML标签生成录音播放器供用户播放。比如: <audio controls src="$audio_address"></audio>
